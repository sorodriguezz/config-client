...
- Soporte para diferentes tipos de servidores de configuración (por defecto y Spring Config Server)
...
- **default**: Usa parámetros de ruta en la URL (ej. `/application/profile`). Adecuado para servidores de configuración genéricos.
- **spring-config-server**: Usa parámetros de consulta (ej. `?repo=...&application=...&profile=...`). Diseñado para compatibilidad con Spring Cloud Config Server.
...
async function cargarConfig() {
  const configModule = ConfigClientModule.forRoot(
    "https://tu-config-server-url/config",
    [
      {
        type: "default",
        application: "nombre-de-tu-aplicacion",
...
- `type`: El tipo de servidor de configuración ('default' o 'spring-config-server')
- `repo`: El nombre del repositorio (requerido para 'spring-config-server', opcional para 'default')
